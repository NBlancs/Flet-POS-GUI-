# Legend 
# Put = // if done
# Put = / if on going task
# Put = x if skip (cause it's hard)


# Major Changes
    # Change the .txt file to csv. file (Basically change the database so that it can be accessible with excel sheets) //
    # Try to change the picture of the products make it work x - Ask Khalifa Tommorow
...
# Design Changes
    # Change the color palette of the product // 
    #  Make the user and password to certain width so that it doesn't extend // 
...
with open("accounts.csv") as file: # TO BE USED WHEN CHECKING FOR MATCHING ACCOUNTS
    for line in file:
        if "*" in line:
            continue
        line.strip()
...
def refreshProducts():
    ...
    with open("products.csv") as file: # THIS IS WHERE I FILL THE PRODUCTS IN I GUESS(?)
        for line in file:
            if "*" in line:
                continue
            line.strip()
...
appName = "Noel's Fruitty Store" # THIS IS THE APP NAME YEAH AS IT SAYS RIGHT THERE
...
userTxtField = flet.TextField(label = "Username", width= 500)
    ...
    def route_change(route):
        ...
        if page.route == "/auth":
            ...
            page.views.append(flet.View(route = "/auth",
                                        appbar = flet.AppBar(title = flet.Row(controls = [flet.Text("Fruitty Store",
                                                                                                    expand = True),
                                                                                          theme_button]),
                                        ...
                                        controls = [
                                            flet.Image(src="https://cdn-icons-png.flaticon.com/512/3194/3194766.png",
                                                 width=140,
                                                 height=140),
                                            ...
                                            flet.Text(appName,
                                                    ...
                                                    userTxtField,
                                                    passTxtField,
                                                    flet.FilledButton(width=130,text = "Log In",
        ...
        # MAIN PAGE, WHERE, AFTER LOGGING IN, YOU GET TO CHOOSE WHETHER TO START NEW TRANSAC, CHECK TRANSACS, OR LOG OUT
            ...
            page.views.append(flet.View(route = "/main",
                                        appbar = flet.AppBar(title = flet.Row(controls = [flet.Text(value = f'Welcome, {userTxtField.value}!',
                                                                                                    expand = True),
                                                                                          theme_button]),
                                                             bgcolor = flet.colors.BLUE_200),
                                        vertical_alignment = flet.MainAxisAlignment.CENTER,
        ...
        # THE PAGE YOU THOUGHT POS WAS ALL ABOUT.... THE BUY BUY THINGY PAGE
            ...
            def ItemCard(product:Product, click: Callable):
                return flet.Container(bgcolor = flet.colors.LIGHT_BLUE_ACCENT_200,
                                      border_radius = 5,
                                      padding = 20,
                                      margin = 2,
            ...
            def on_sale(_):
                ...
                def z(_):
                    dialog.title = flet.Image(src = 'https://cdn-icons-png.flaticon.com/512/1577/1577330.png',
                                                             width = 400,
                                                             height = 400,
                                                             fit = True)
                ...
                def on_confirm(_):
                    ...
                    if payField.value == "":
                        ...
                        dialog.title = flet.Image(src = 'https://cdn-icons-png.flaticon.com/512/2238/2238026.png',
                                                  width = 400,
                                                  height = 400,
                                                  fit = True)
                        ...
                        dialog.title = flet.Image(src = 'https://cdn4.iconfinder.com/data/icons/eshop/403/37-512.png',
                                                  width = 400,
                                                  height = 400,
                                                  fit = True)
                ...
                def refresh_dialog():
                    return flet.AlertDialog(title = flet.Image(src = 'https://cdn-icons-png.flaticon.com/512/1027/1027650.png',
                                                             width = 400,
                                                             height = 400,
                                                             fit = True),
                                          content = flet.Column(alignment = flet.MainAxisAlignment.CENTER,
                ...
                if len(cartItems) == 0:
                    dialog.title = flet.Image(src = 'https://thumbs.dreamstime.com/b/add-to-shopping-cart-icon-online-buying-vector-no-stop-sign-supermarket-basket-symbol-prohibited-ban-market-146911099.jpg',
                                              width = 400,
                                              height = 400,
                                              fit = True)
